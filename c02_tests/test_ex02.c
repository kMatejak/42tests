...
int	main(void)
{
	...
	a = ft_str_is_alpha(str_a);
	b = ft_str_is_alpha(str_b);
	c = ft_str_is_alpha(str_c);
	d = ft_str_is_alpha(str_d);
	e = ft_str_is_alpha(str_e);
	...
	if (d == 0)
	{
		printf(GREEN "String d: >>%s<<\ncontains alpha with spaces." RESET "\n\n", str_d);
	}
	else
	{
